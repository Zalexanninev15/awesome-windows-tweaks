...
:Start
	...
	Color 0f
	if not exist "%~dp0Work" echo Не найдена рабочая папка Work рядом с программой, будет выполнен выход. && timeout /t 3 >nul && exit
	cd /d "%~dp0Work"
...
rem Если не существует WMIC/nhmb
	...
	if not exist nhmb.exe %ch% {0c} Нет файла nhmb.exe в папке Work. Перекачайте полный архив DefenderKiller с Я. Диска{\n #}&&timeout /t 5 >nul&&exit
...
:CreateBackupDefender
	...
	if "%xOS%"=="x86" %ch%    {04} Создание резервной копии недоступно на x32{\n #}&&pause&&cls&&goto Start
