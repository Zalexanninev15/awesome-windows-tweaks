...
rmdir C:\Windows\system32\adminrightstest > nul
fsutil behavior set disabledeletenotify 1 >> me_log.txt
IF ERRORLEVEL 1 (
	echo WARNING: Failed to apply HDD drive behavior patch. Check me_log.txt for details.
	pause
) else (
	echo HDD drive behavior patch has been applied
)
