...
rem Make a copy of the register in the C: /RegBackup/Backup.reg
rem Disable UAC
rem Disable Windows 10 to restore itself from the Windows Update
rem Customize privacy
rem Tweaks registry - Delete keys start clutter
rem Disable unwanted group policies
rem Remove telemetry data collection disables Cortana
rem Disconnect and remove the service
rem Disable and remove internet search bing and indices
rem Disable unnecessary task scheduler
rem Block unwanted Web sites to the hosts file
rem Delete Demo content
rem Disable unwanted features
rem Remove OneDrive and other applications
rem Remove 3D objects
rem Remove CameraRollLibrary
rem Remove from the menu MyComputer Music, Pictures, Videos, Documents, Downloads, Desktop
rem Rename the computer
rem Set automatic logon
rem Remove wallpaper on the login screen
rem Show a Computer shortcut on your desktop
rem Underline keyboard shortcuts and access keys
rem Set Windows Explorer to start
rem Hide search window from the taskbar
rem Disable XAML lists of applications in the Start menu
rem Show file extensions
rem Show hidden extension
rem Use the Photo Viewer, to open the photo files
rem Turn off Sticky Keys by pressing SHIFT 5 times
rem Disable Filter Keys when SHIFT is pressed for 8 seconds
rem Change the hour and date format 24 hours, the metric system
rem Google - Search Default
rem Upgrading Windows - only receive directly from Microsoft
rem AutoUpdate Windows - ask to download and install (you can disable the automatic update)
rem Disable shared resources Wi-Fi network
rem Open to the current folder in the left pane in Windows Explorer
rem Prevent the creation of shortcuts in the Recent folder
rem Remove Previous Versions tab that appears when you right-click the file> Properties
rem Delay pop-up taskbar - 10 seconds
rem Disable Notification Center
rem Runs in the form of a cleaning operation, recovery and reset the database - all installed Windows updates are permanent and can not be removed
rem Clear temporary files
rem Clear autostart registry
rem Clear the Startup folder
rem Reloading
COPY "%WINDIR%\system32\drivers\etc\hosts" "%WINDIR%\system32\drivers\etc\hosts.backup.txt"
ATTRIB -r "%WINDIR%\system32\drivers\etc\hosts"
SET HOSTS=%WINDIR%\System32\drivers\etc\hosts
