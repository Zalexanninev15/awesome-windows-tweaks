...
rem Проверяем, чтобы в пути не было скобок
	if not exist "%~dp0Work" echo Не найдена рабочая папка Work рядом с программой, будет выполнен выход. && timeout /t 7 >nul && exit
	echo "%~dp0" | findstr /c:"(" /c:")" > nul && echo Путь до .bat содержит скобки, исправьте путь и запустите программу повторно. && timeout /t 7 >nul && exit
...
rem Размер программы в зависимости от наличия службы, расположение по центру экрана и название
	...
	if not exist "%SystemRoot%\System32\wbem\WMIC.exe" %ch% {04} Не найден компонент WMIC, который есть в оригинальной Windows по-умолчанию.{\n #} Работоспособность данной утилиты невозможна без наличия данного компонента.{\n #}{04} Будет выполнен выход.{\n #}&& timeout /t 7 >nul && exit
	if not exist nhmb.exe %ch% {0c} Нет файла nhmb.exe в папке Work. Перекачайте полный архив DefenderKiller{\n #}&&timeout /t 7 >nul&&exit
...
(
	...
	%ch% {0f}3{#} - {03}Проверить обновления DK {08}[{0e}%Version%{08} от {03}%DateProgram% {08}^/ By {02}Vlado{08}]{\n #}
	%ch% {0f}4{#} - {08}Управление (восстановление/ГП){\n #}
...
rem Дополнительная проверка удалялся ли защитник ранее по ветке реестра, которая удаляется в процессе удаления.
	reg query "HKLM\Software\Microsoft\Windows Defender\Features" >nul 2>&1 && echo >nul || goto SkipTamperProtection
	
rem Проверяем, отключена ли защита от подделки, если да - сразу переходим к предложению о создании рез. копии
	2>nul reg query "HKLM\Software\Microsoft\Windows Defender\Features" /v "TamperProtection" | findstr /c:"0x0" /c:"0x4" >nul 2>&1 && %ch%    {0a} Защита от подделки отключена^^!{\n #}{\n #}&&goto TamperNotExistOrDisable

rem Проверяем билд Windows, если это 1809 и ниже, где нет Защиты от подделки, сразу переходим к предложению о создании рез. копии
	...
	if "!BuildOS!" leq "17763" %ch%    {0a} Защита от подделки на этой версии не найдена^^! {08}[версия 1809 и ниже]{\n #}{\n #}&& goto TamperNotExistOrDisable
	
rem Если это 1909 - запускаем оснастку Безопасности и просим выключить все параметры, т.к. на этой Windows другая последовательность nircmd
	if "!BuildOS!" equ "18362" (
	%ch%    {0a} Защита от подделки на этой версии найдена^^!{\n #}{\n #}
	%ch%    {08} Отключите все параметры вручную и нажмите любую клавишу...{\n #}{\n #}
	qprocess SecHealthUI.exe >nul 2>&1 || explorer windowsdefender://ThreatSettings
	timeout /t 2 >nul
	2>nul reg query "HKLM\Software\Microsoft\Windows Defender\Real-Time Protection" /v "DisableRealtimeMonitoring" | find /i "0x1" >nul 2>&1 && %ch%    {02} Защита в режиме реального времени отключена^^!{\n #}{\n #}|| %ch%    {04} Отключите Защиту в режиме реального времени{\n #}&&timeout /t 2 >nul && cls && goto DeleteDefender
	2>nul reg query "HKLM\Software\Microsoft\Windows Defender\Features" /v "TamperProtection" | findstr /c:"0x0" /c:"0x4" >nul 2>&1 && %ch%    {0a} Защита от подделки отключена^^!{\n #}{\n #}&&goto TamperNotExistOrDisable||%ch%    {04} Отключите Защиту от подделки{\n #}&&timeout /t 2 >nul && cls && goto DeleteDefender)
...
rem Проверяем Dev ли Windows
	for /f "tokens=1* delims==Майкрософт " %%A IN ('wmic os get caption /Format:List ^| FIND "="') do set "ThisDevOrNotVersionWindows=%%~B"

	%ch%    {08} Будут выключены все параметры Defender в автоматическом режиме.{\n #}
	%ch%    {04} Главное, что необходимо отключить - {4f}Защита от подделки{\n #}{\n #}
	%ch%    {0c} После нажатия программа будет ждать отключения "Защиты от подделки"{\n #}
	%ch%    {0c} Если у Вас недоступна страница Безопасности - закройте её{\n #}
	%ch%    {08} Нажмите любую клавишу для отключения{\n #}
	
	pause>nul && cls
	explorer windowsdefender://ThreatSettings
	timeout /t 2 /nobreak >nul
rem Выключаем все параметры с помощью nircmd с различными комбинациями в зависимости от того Dev ли это или нет
...
:TamperProtectionStatus
	...
	reg query "HKLM\Software\Microsoft\Windows Defender\Features" /v "TamperProtection" | findstr /c:"0x0" /c:"0x4" >nul 2>&1 && cls && %ch%    {0a} Защита от подделки отключена^^!{\n #}{\n #}|| goto TamperProtectionStatus
	) || (
	%ch%    {0c} Функция Защита от подделки {4f}не{0c} отключена^^!{\n #}{\n #}
	%ch%    {08} 1 - {04}Я не могу отключить защиту от подделки{\n #}
	%ch%    {08} Выбирайте 1, если действительно не можете отключить данную функцию{\n #}
	%ch%    {08} Такое может быть, если страница параметров недоступна из-за ранее отключенного защитника{\n #}
	%ch%    {04} ВЫБИРАЯ 1, ВЫ ПРОПУСКАЕТЕ СОЗДАНИЕ РЕЗЕРВНОЙ КОПИИ ЗАЩИТНИКА{\n #}
	echo.
	%ch%    {08} 2 - Вернуться обратно в главное меню{\n #}
	choice /c 12 /n /m " "
...
rem Информация по политикам : https://learn.microsoft.com/ru-ru/windows/client-management/mdm/policy-csp-admx-microsoftdefenderantivirus
	if defined NoGP %ch%    {08}Пропуск отключения защитника, т.к. не найдено ГП{\n #}&&goto NotGP
...
REM rem Если это вторая [повторная] проходка удаления - пропускаем добавление в исключения
...
rem В переменную получаем ссылку для DefenderStop в зависимости от разрядности OS
	if "%xOS%"=="x86" (
	...
	%ch%    {08} Скачиваем DefenderStop %xOS%{\n #}
	...
	%ch%    {03} Используем DefenderStop %xOS% для отключения служб защитника{\n #}{\n #}
...
rem                                                                   =================================
	...
	%ch%    {08} Скачиваем Unlocker by Eject{\n #}
	curl -g -k -L -# -o "Unlocker.exe" "https://i.getspace.eu/cloud/s/AK8zrMPcnP3dHZk/download"
	)
	
	%ch%    {0c} Выполняется удаление с помощью Unlocker by Eject{\n #}
...
rem Применяем политику по отключению защитника уже после удаления, чтобы программы не зависали.
	if not defined NoGP (
...
rem Проверяем создавался ли бекап, если нет - спрашиваем точно ли удалить папки из WinSxS
	reg query "HKLM\Software\DefenderKiller" >nul 2>&1 && (
	echo >nul
	) || (
	nhmb "Вы решили не создавать резервную копию.\n\nУдаление папок нарушит установку некоторых накопительных обновлений Windows, поэтому, если Вы обновляете Windows, откажитесь от удаления папок!\n\nВы точно хотите удалить папки Windows Defender из WinSxS?" "DK" "Warning|YesNo|DefButton2"
	if errorlevel 7 (
	%ch%    {04} Вы решили пропустить удаление папок из WinSxS{\n #}
...
rem Проверяем создавался ли бекап, если да - пропускаем удаление приложений
	...
	%ch%    {09} Удаление Безопасность Windows{\n #}
	set "NameSecHealth="
...
:AppRepSys
	%ch%    {09} Удаление SmartScreen защитника Windows{\n #}
	set "NameAppRep="
...
rem Освобождённое место на диске
	...
	echo.
	nhmb "Защитник Windows не удален.\nЕсли появляется это сообщение, выполните перезагрузку компьютера и повторите попытку удаления.\n\nПовторить удаление защитника?\n" "DK" "Information|YesNo"
...
rem Размер окна в зависимости от наличия службы
	...
	if not exist "%SystemDrive%\Program Files\Windows Defender" (%ch% {02} C:\Program Files\Windows Defender {08}- основная папка защитника 1{\n #}) else (%ch%  {4f}C:\Program Files\Windows Defender{08} - основная папка защитника 1{\n #})
	if not exist "%AllUsersProfile%\Microsoft\Windows Defender" (%ch% {02} C:\ProgramData\Microsoft\Windows Defender {08}- основная папка защитника 2{\n #}) else (%ch%  {4f}C:\ProgramData\Microsoft\Windows Defender{08} - основная папка защитника 2{\n #})
	echo.
...
:CreateBackupDefender
	if exist "%SystemDrive%\WDefenderBackup" %ch%    {02} Резервная копия защитника уже создана{\n #}&&exit /b
...
rem Проверяем наличие интернета, чтобы использовать DefenderStop перед созданием резервной копии [нужен, т.к. без него не копируется папка защитника, из-за включённых служб]
	if not exist DefenderStop.exe (
	%ch%    {08} Скачиваем DefenderStop %xOS%/Unlocker{\n #}
	ping www.google.nl -n 1 -w 1000 |>nul find /i "TTL=" && curl -g -k -L -# -o "DefenderStop.exe" "%LinkDefStop%"
	curl -g -k -L -# -o "Unlocker.exe" "https://i.getspace.eu/cloud/s/AK8zrMPcnP3dHZk/download"
	) || (
	%ch%    {0c} Невозможно создать резервную копию, нет подключения к интернету{\n #}
	%ch%    {0c} Невозможно скачать DefenderStop{\n #}{\n #}
	%ch%    {08} Нажмите любую клавишу. Вы будете возвращены в меню без продолжения удаления.{\n #}
	pause && cls && goto Start)

rem Используем несколько раз для уверенности выполнения
	%ch%    {03} Используем DefenderStop %xOS% 3 раза для отключения служб защитника{\n #}{\n #}
	nircmd exec hide DefenderStop.exe
	nircmd exec hide DefenderStop.exe
	nircmd exec hide DefenderStop.exe
	
	%ch%    {03} Используем Unlocker для разблокировки папок защитника{\n #}{\n #}
...
rem Получаем версию Windows, выводим ее, если папка не скопировалась
	for /f "tokens=1* delims==Майкрософт " %%A IN ('wmic os get caption /Format:List ^| FIND "="') do set "VersionWin=%%~B"
...
rem Проверка после копирования главной папки, скопировалась ли ...
	...
	%ch% {08} Ваша версия Windows - {03}%VersionWin% {08}^| {03}%NumberWin%{\n #}
...
:ManageDefender
	%ch% {08} 1{#} - Восстановить защитник {08}[из бэкапа]{\n #}
	%ch% {08} 2{#} - Восстановить только папки из WinSxS {08}[из бэкапа]{\n #}
	%ch% {08} 3{#} - Откатить ГП защитника {08}[только политики, без восстановления защитника]{\n #}
	echo.
	%ch% {0e} [Enter]{#} - {08}Вернуться в главное меню{\n #}
	set "input="
	set /p input=
	if not defined input   cls && goto Start
	if "%input%"=="1"    nhmb "Полное восстановление защитника в исходное состояние как было до удаления." " DefenderKiller" "Information" & goto RestoreDefender
	if "%input%"=="2"    nhmb "В теории должно хватить для того, чтобы центр обновления стал корректно работать, если возникли проблемы с ним после использования программы.\nВыберите папку бэкапа, которую создавали ранее в процессе удаления." " DefenderKiller" "Information" & goto RestoreDefender
	if "%input%"=="3"    call :RestoreGP && call :RestoreGP && timeout /t 1 >nul && cls && goto ManageDefender

	) else (
		echo. && %ch%    {0c}Такой функции не существует {\n #}&& echo.
...
rem Для корректного отображения диалогового окна, т.к. программа запущена от TI
	...
	%ch%  {4f}Убедитесь, что выбранная рез. копия была создана на этой же версии Windows{\n #}
	
	set "BackupFolder="
	for /f %%a in ('powershell -c "(New-Object -COM 'Shell.Application').BrowseForFolder(0, 'Выберите папку с ранее созданной резервной копией Windows Defender. После выбора папки будет задан вопрос о восстановлении защитника.', 0, 0).Self.Path"') do set "BackupFolder=%%a"
	echo.
...
:CheckUpdate
rem Windows 7/8/8.1 не поддерживает функционал обновлений
	ver | findstr /c:"6.3" /c:"6.2" /c:"6.1" >nul && (
	%ch% {0c} Данная версия Windows не поддерживает этот функционал.{\n #}
	%ch% {0c} Последняя версия доступна на сайте программы.{\n #}{\n #}
	%ch% {08} Для возврата в главное меню нажмите любую клавишу.{\n #}
	pause>nul
...
rem Проверяем наличие интернет-соединения
	ping www.google.nl -n 1 -w 1000 |>nul find /i "TTL="|| cls && %ch% {04} Ошибка проверки, нет интернет-соединения.{\n #}&&timeout /t 3 >nul && cls && goto Start
